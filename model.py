...
ONLY COMMENTS PROVIDED FOR ADAPTIOM DUE TO NEW DECODER.
...
class STM(nn.Module):
    def __init__(self, new_arch=False):
        ...
        """USE ORIGINAL OR ADAPTED DECODER"""
        if new_arch:
            self.Decoder = Decoder_NEW(256)
        else:
    ...
    def Pad_memory(self, mems, num_objects, K):
        pad_mems = []
        for mem in mems:
            pad_mem = torch.zeros(1, K, mem.size()[1], 1, mem.size()[2], mem.size()[3])
    ...
    def memorize(self, frame, masks, num_objects): 
        ...
        # make Batch
        B_ = {arg: torch.cat(B_list[arg], dim=0) for arg in B_list.keys()}
